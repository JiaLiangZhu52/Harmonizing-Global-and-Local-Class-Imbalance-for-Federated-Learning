...
def test_img(net_g, datatest, label, args):
    ...
    l = len(data_loader)
    false = np.zeros((args.num_classes, 1))
    false_all = np.zeros((args.num_classes, 1))
    for idx, (data, target) in enumerate(data_loader):
        ...
        pred_one_hot = label_binarize(y_pred.data.cpu().numpy(), [i for i in range(args.num_classes)])
        target_one_hot = label_binarize(target.data.view_as(y_pred).cpu().numpy(), [i for i in range(args.num_classes)])
    ...
    auc_all = np.zeros((1, args.num_classes))
    accuracy = 100.00 * correct / len(data_loader.dataset)
    for i in range(len(false)):
        ac_temp = (false_all[i, 0] - false[i, 0]) / false_all[i, 0]
        print('{:.4f}'.format(ac_temp))
        auc_all[0, i] = ac_temp
    print('AUC Score: {:.6f}, AUC Score New: {:.6f}, Ac.M: {:.4f}'.format(auc_res, auc_res_new, np.mean(auc_all[0, 0:3])))
    if args.verbose:
